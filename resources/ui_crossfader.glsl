void main(void) {
    ...
    f_color0 = vec4(0.);
    if(iSelection) {
        float df = max(rounded_rect_df(frag,vec2(45., 50.), vec2(260., 400.), RADIUS), 0.);
        if(df >= 1.0)
            discard;
        f_color0 = composite(f_color0, vec4(dataColor(ivec3(3, 0, 0)), rounded_rect_df(frag,vec2(45., 50.), vec2(260.,400.), RADIUS) <= 1.));
        f_color0.rgb = mix(f_color0.rgb, dataColor(ivec3(4, 0, 0)), inBox(frag.xy, slider_pos - slider_size, slider_pos + slider_size));
    } else {
        f_color0 = composite(f_color0, fancy_rect(frag,vec2(45, 50.), vec2(260., 400.), iSelected == 17 || iSelected == 18));
        float df = max(rounded_rect_df(frag,vec2(45., 50.), vec2(260., 400.), RADIUS), 0.);
